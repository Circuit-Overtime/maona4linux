...
When available, the default is packed 24-bit (`S24_3LE`), mono, 48 kHz. This preserves capture headroom while remaining compatible with common Linux audio and video workflows. Higher advertised sample rates are useful for targeted testing, but are not automatically better for voice recording.
